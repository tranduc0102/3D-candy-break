...
public class ControlStick : MonoBehaviour
{
    ...
    public void ChangeStick(Transform newStick)
    {
        ...
        stick.localEulerAngles = new Vector3(stick.localEulerAngles.x, stick.localEulerAngles.y, -60f);
        if (!stick.TryGetComponent<Stick>(out Stick s))
        { 
            stick.gameObject.AddComponent<Stick>();
        }
     ...
     private void HandleFingerDown(LeanFinger finger)
        {
            ...
            stick.DOMove(new Vector3(position.x, 0.15f/12f, position.z + 0.025f) * 12f, 0.5f).SetEase(Ease.Linear).OnComplete(() =>
            {
                isDown = true;  
            });
